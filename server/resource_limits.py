...
def execute_with_timeout(command: List[str], timeout_seconds: int = 30,
    ...
    logger.debug(f"Executing command with timeout {timeout_seconds}s: {' '.join(command)}")
    ...
    try:
        ...
        exit_code = process.returncode if process.returncode is not None else 124  # Timeout exit code
        ...
        logger.debug(f"Command completed: exit_code={exit_code}, elapsed={elapsed_ms}ms, "
...
def get_tool_timeout(tool_name: str, default_timeout: int = 30) -> int:
    ...
    if timeout_str:
        try:
            ...
            if timeout > 0:
                ...
                logger.warning(f"Invalid timeout value for {tool_name}: {timeout_str}, using default")
        except ValueError:
            logger.warning(f"Invalid timeout format for {tool_name}: {timeout_str}, using default")
    ...
    if global_timeout_str:
        try:
            ...
            logger.warning(f"Invalid global timeout format: {global_timeout_str}, using default")
...
def get_output_limit(tool_name: str, default_limit: int = 1024 * 1024) -> int:
    ...
    if limit_str:
        try:
            ...
            if limit > 0:
                ...
                logger.warning(f"Invalid output limit for {tool_name}: {limit_str}, using default")
        except ValueError:
            logger.warning(f"Invalid output limit format for {tool_name}: {limit_str}, using default")
    ...
    if global_limit_str:
        try:
            ...
            logger.warning(f"Invalid global output limit format: {global_limit_str}, using default")
