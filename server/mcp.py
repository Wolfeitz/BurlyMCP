...
class MCPResponse:
    ...
    def _truncate_output(self, max_length: int = 10000):
        ...
        if len(self.stdout) > max_length:
            self.stdout = self.stdout[:max_length - len(truncation_indicator)] + truncation_indicator
            if "stdout_trunc" not in self.metrics:
                self.metrics["stdout_trunc"] = len(self.stdout)
        
        if len(self.stderr) > max_length:
            self.stderr = self.stderr[:max_length - len(truncation_indicator)] + truncation_indicator
...
class MCPProtocolHandler:
    ...
    def write_response(self, response: MCPResponse) -> None:
        ...
        try:
            ...
            try:
                ...
                fallback_json = json.dumps(fallback_response.to_json(), separators=(',', ':'))
    ...
    def create_error_response(self, error_msg: str, summary: str = "Error") -> MCPResponse:
        """
    ...
    def handle_request(self, request: MCPRequest) -> MCPResponse:
        ...
        try:
            ...
            if request.method == MCPMethod.LIST_TOOLS.value:
                ...
                logger.debug(f"Tool {request.name} completed with status: {'success' if response.ok else 'failure'}")
                return response
            else:
    ...
    def _handle_list_tools(self) -> MCPResponse:
        ...
        return self.create_success_response(
            summary=f"Available tools: {len(tools)} tools found",
            data={"tools": tools}
        )
    ...
    def _handle_call_tool(self, request: MCPRequest) -> MCPResponse:
        ...
        if not request.name:
            return self.create_error_response(
                "Tool name is required for call_tool",
                "Missing tool name"
            )
        ...
        try:
            while True:
                try:
                    ...
                    logger.debug(f"Request completed in {response.metrics.get('elapsed_ms', 0)}ms")
                    ...
                    logger.error(f"Unexpected error in protocol loop: {e}", exc_info=True)
                    error_response = self.create_error_response(
