...
class ToolRegistry:
    ...
    def _tool_requires_confirm(self, tool_name: str) -> bool:
        ...
        """
        return self.tool_characteristics.get(tool_name, {}).get("requires_confirm", False)
    ...
    def _docker_ps(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            cmd = ["docker", "ps", "--format", "table {{.ID}}\t{{.Image}}\t{{.Command}}\t{{.CreatedAt}}\t{{.Status}}\t{{.Ports}}\t{{.Names}}"]
            
            # Get configured timeout and output limits for docker_ps
            timeout = get_tool_timeout("docker_ps", default_timeout=30)
            output_limit = get_output_limit("docker_ps", default_limit=512 * 1024)  # 512KB for docker output
            ...
            if result.success and result.exit_code == 0:
                ...
                return ToolResult(
                    ...
                    data={
                        ...
                        "original_output_size": result.original_stdout_size + result.original_stderr_size
                    },
                ...
                if result.timed_out:
                    ...
                    if "permission denied" in error_msg.lower():
                        ...
                        summary = f"Docker command failed with exit code {result.exit_code}"
    ...
    def _disk_space(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            output_limit = get_output_limit("disk_space", default_limit=256 * 1024)  # 256KB for filesystem output
            ...
            if result.success and result.exit_code == 0:
                    ...
                    for line in lines[1:]:
                        if line.strip():  # Skip empty lines
                            ...
                            if len(parts) >= 7:
                                ...
                                avail = parts[4]
                                use_percent = parts[5]
                                mounted_on = ' '.join(parts[6:])  # Join remaining parts for mount point
                ...
                if high_usage_fs:
                    ...
                    summary = f"Found {fs_count} filesystems, all with healthy usage levels"
    ...
    def _blog_stage_markdown(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not content.startswith('---\n'):
                validation_errors.append("Missing YAML front-matter start delimiter (---)")
            else:
                # Find the end of front-matter
                end_delimiter_match = re.search(r'\n---\n', content[4:])
                if not end_delimiter_match:
                    validation_errors.append("Missing YAML front-matter end delimiter (---)")
                else:
            ...
            if "date" in front_matter:
                ...
                if isinstance(date_value, str):
                    if not re.match(r'^\d{4}-\d{2}-\d{2}', date_value):
                        validation_errors.append("Date field should be in YYYY-MM-DD format")
            ...
            for source_file in files_to_publish:
                try:
                    ...
                    try:
                        ...
                        copy_errors.append(f"Path traversal detected for {rel_path}: {str(e)}")
                        continue
                    ...
                    copy_errors.append(f"Permission denied copying {rel_path}: {str(e)}")
                except OSError as e:
