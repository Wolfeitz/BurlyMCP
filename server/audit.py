...
class AuditLogger:
    ...
    def log_tool_execution(self, tool_name: str, args: Dict[str, Any],
                          mutates: bool, requires_confirm: bool,
                          status: str, exit_code: int, elapsed_ms: int,
                          stdout_truncated: int = 0, stderr_truncated: int = 0,
                          caller: str = "mcp_client") -> None:
        """
    ...
    def log_security_violation(self, violation_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(f"Security violation logged: {violation_data.get('violation_type', 'unknown')}")
    ...
    def _write_audit_record(self, record: AuditRecord) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to write audit record to {self.log_file_path}: {str(e)}")
            raise
    ...
    def _hash_sanitized_args(self, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            args_json = json.dumps(sanitized_args, sort_keys=True, separators=(',', ':'))
    ...
    def _sanitize_args(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in args.items():
            ...
            if any(sensitive in key_lower for sensitive in ['password', 'token', 'secret', 'key', 'auth']):
                ...
                sanitized[key] = [self._sanitize_args(item) if isinstance(item, dict) else item for item in value]
            else:
    ...
    def _get_sensitive_env_vars(self) -> List[str]:
        ...
        default_sensitive = [
            ...
            'GOTIFY_TOKEN', 'GOTIFY_URL', 'API_KEY', 'API_SECRET',
            ...
            with open(self.log_file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    try:
                        ...
                        # Parse timestamp
                        record_time = datetime.fromisoformat(record['ts'].replace('Z', '+00:00')).timestamp()
