...
def validate_path_within_root(file_path: str, root_directory: str, 
                             operation_name: str = "file_operation") -> str:
    """
    ...
    try:
        ...
        if not abs_file_path.startswith(abs_root + os.sep) and abs_file_path != abs_root:
            ...
            raise SecurityViolationError(
                f"Path traversal detected: '{file_path}' resolves outside root directory '{root_directory}'"
            )
        
        logger.debug(f"Path validation passed for {operation_name}: {file_path} -> {abs_file_path}")
...
def validate_blog_stage_path(file_path: str) -> str:
    ...
    return validate_path_within_root(
        file_path=file_path,
        root_directory=blog_stage_root,
        operation_name="blog_stage"
    )
...
def log_security_violation(violation_type: str, operation: str, 
                          attempted_path: str, root_directory: str,
    ...
    logger.warning(f"SECURITY VIOLATION: {violation_type} in {operation}", extra=violation_data)
