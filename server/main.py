...
from .policy import PolicyLoader, SchemaValidator, ToolRegistry as PolicyToolRegistry
...
def load_configuration() -> Dict[str, Any]:
    ...
    config = {
        ...
        "blog_publish_root": os.environ.get("BLOG_PUBLISH_ROOT", "/app/data/blog/public"),
...
def initialize_policy_engine(config: Dict[str, Any], logger: logging.Logger) -> PolicyToolRegistry:
    """
    ...
    try:
        ...
        logger.info(f"Policy loaded successfully with {len(policy_loader.get_all_tools())} tools")
...
def initialize_audit_system(config: Dict[str, Any], logger: logging.Logger) -> AuditLogger:
    """
...
def initialize_notification_system(config: Dict[str, Any], logger: logging.Logger) -> bool:
    """
    ...
    try:
        ...
        if status["enabled"]:
            available_providers = [p["name"] for p in status["providers"] if p["available"]]
        ...
        if dir_path:
            try:
                ...
                if dir_key == "blog_publish_root" and not os.access(dir_path, os.W_OK):
                    validation_errors.append(f"Publish directory not writable: {dir_path}")
            except OSError as e:
...
def main() -> None:
    ...
    try:
        ...
        _logger.info(f"Notifications: {'enabled' if config['notifications_enabled'] else 'disabled'}")
        ...
        _logger.info(f"Notifications: {'enabled' if notifications_available else 'disabled'}")
