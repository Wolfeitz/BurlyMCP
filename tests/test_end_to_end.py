...
class TestMCPProtocolEndToEnd:
    ...
    def test_list_tools_complete_cycle(self):
        ...
        expected_tools = ["docker_ps", "disk_space", "blog_stage_markdown", 
                         "blog_publish_static", "gotify_ping"]
    ...
    def test_invalid_method_complete_cycle(self):
        ...
        assert response.need_confirm is False
        assert "Method not supported" in response.summary or "Unsupported method" in response.summary
...
"""
    ...
    def test_blog_publish_confirmation_workflow_complete_cycle(self, temp_dir):
        ...
        with patch.dict(os.environ, {
            ...
            assert "confirmation" in response.summary.lower() or "ready to publish" in response.summary.lower()
    ...
    def test_nonexistent_tool_complete_cycle(self):
        ...
        assert json_response["ok"] is False
        assert "error" in json_response
        assert "not found" in json_response["error"].lower() or "unknown" in json_response["error"].lower()
...
class TestMCPProtocolLoop:
    ...
    def test_request_parsing_invalid_json(self):
        ...
        invalid_requests = [
            ...
            '{"method": "list_tools", "extra": {"deeply": {"nested": {"object": "value"}}}}' * 100,  # Too complex
        ]
    ...
    def test_rate_limiting(self):
        ...
        self.mcp_handler._request_times = [current_time] * self.mcp_handler._max_requests_per_minute
    ...
    def test_security_input_size_limit(self):
        ...
        large_request = '{"method": "list_tools", "data": "' + 'x' * (1024 * 1024 + 1) + '"}'
        ...
        for request_data in readonly_requests:
            ...
            with patch('server.tools.execute_with_timeout') as mock_execute:
                mock_execute.return_value = ExecutionResult(
                    ...
                    timed_out=False, elapsed_ms=100, stdout_truncated=False,
                    stderr_truncated=False, original_stdout_size=11, original_stderr_size=0
                )
        ...
        with patch.dict(os.environ, {
            ...
            for confirm_value in confirmation_values:
                request = MCPRequest.from_json({
                    "method": "call_tool",
                    "name": "blog_publish_static",
                    "args": {"source_files": ["test.md"], "_confirm": confirm_value}
                })
