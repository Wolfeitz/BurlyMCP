...
class TestPathTraversalPrevention:
    ...
    def test_validate_path_within_root_safe_paths(self, temp_dir):
        ...
        for safe_path in safe_paths:
            ...
            assert os.path.normpath(result) == os.path.normpath(os.path.join(root_dir, safe_path))

    @patch('server.audit.get_audit_logger')
    def test_validate_path_within_root_traversal_attacks(self, mock_audit_logger, temp_dir):
    ...
    @patch('server.audit.get_audit_logger')
    def test_validate_path_within_root_symbolic_links(self, mock_audit_logger, temp_dir):
        ...
        try:
            ...
            result = validate_path_within_root(str(symlink_path), root_dir, "test_operation")
    ...
    @patch('server.audit.get_audit_logger')
    def test_blog_path_validation_functions(self, mock_audit_logger, monkeypatch, temp_dir):
    ...
    def test_blog_path_validation_missing_config(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match="BLOG_STAGE_ROOT environment variable not configured"):
            validate_blog_stage_path("post.md")
            
        with pytest.raises(ValueError, match="BLOG_PUBLISH_ROOT environment variable not configured"):
    ...
    def test_path_validation_with_os_errors(self, mock_logger, mock_audit_logger, temp_dir):
...
class TestTimeoutEnforcement:
    ...
    def test_execute_with_timeout_invalid_parameters(self):
        ...
        with pytest.raises(ValueError, match="Max output size must be positive"):
            execute_with_timeout(command=["echo", "test"], timeout_seconds=5, max_output_size=-1)
...
class TestOutputTruncation:
    ...
    def test_truncate_output_very_small_limit(self):
        ...
        result = truncate_output(output, max_size=20, stream_name="test")  # Use very small limit
...
class TestToolSecurityIntegration:
    ...
    @patch('server.audit.get_audit_logger')
    def test_tool_registry_path_traversal_protection(self, mock_audit_logger, temp_dir, monkeypatch):
    ...
    def test_security_violation_notification(self, mock_log_violation, mock_audit_logger, temp_dir, monkeypatch):
...
class TestSecurityEdgeCases:
    ...
    def test_concurrent_security_operations(self, mock_audit_logger, temp_dir):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(validate_path_worker, path) for path in safe_paths]
        ...
        mixed_paths = ["safe.txt", "../../../etc/passwd", "also_safe.txt", "../../dangerous"]
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(validate_path_worker, path) for path in mixed_paths]
            concurrent.futures.wait(futures)
    ...
    def test_resource_exhaustion_protection(self):
        ...
        result = execute_with_timeout(
            command=["python3", "-c", "print('x' * 1000000)"],  # Try to generate 1MB output
            timeout_seconds=5,
