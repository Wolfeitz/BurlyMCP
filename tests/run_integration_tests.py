...
Examples:
  ...
  %(prog)s --parallel 4             # Run tests in parallel
    ...
    parser.add_argument(
        '--no-docker', dest='include_docker', action='store_false',
        help='Skip Docker-related tests'
    )
    parser.add_argument(
        '--no-slow', dest='include_slow', action='store_false',
        help='Skip slow-running tests'
    )
    parser.add_argument(
        '--only-mcp', action='store_true',
        help='Run only MCP protocol tests'
    )
    parser.add_argument(
        '--test-file', metavar='FILE',
        help='Run specific test file (e.g., test_docker_integration.py)'
    )
    parser.add_argument(
        '--test-function', metavar='FUNCTION',
    ...
    parser.add_argument(
        '--timeout', type=int, metavar='SECONDS', default=300,
